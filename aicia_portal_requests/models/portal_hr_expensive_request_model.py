...
class PortalHrExpensiveRequest(models.Model):
    ...
    def _compute_show_solicitar_revision(self):
        is_boss = self.env.user.has_group('aicia_portal_requests.group_equip_boss') or self.env.user.has_group(
            'aicia_portal_requests.group_intern_partner_responsible')
        for rec in self:
    ...
    def _compute_show_project_credit(self):
        for record in self:
            if self.env.user.has_group('aicia_portal_requests.group_director_manager') or self.env.user.has_group('aicia_portal_requests.group_personnel_purchase_responsible') or self.env.user == record.equip_boss  or self.env.user == record.project.responsible_id:
    ...
    def _compute_show_group_credit(self):
        for record in self:
            if self.env.user.has_group('aicia_portal_requests.group_director_manager') or self.env.user.has_group('aicia_portal_requests.group_personnel_purchase_responsible') or self.env.user == record.equip_boss:
    ...
    def action_approve(self):
        if self.env.user.has_group('aicia_portal_requests.group_equip_boss') and self.status == 'approved_by_boss_group':
            self.status = 'approved_purchase_responsible'
            user_to_notify = self.env['res.users'].search(
                [('work_group_ids', 'in', [self.env.ref('aicia_portal_requests.group_personnel_purchase_responsible').id])])
        ...
        elif self.env.user.has_group('aicia_portal_requests.group_personnel_purchase_responsible') and self.status == 'approved_purchase_responsible':
            ...
            if self.is_more:
                ...
                user_to_notify = self.env['res.users'].search(
                    [('work_group_ids', 'in', self.env.ref('aicia_portal_requests.group_director_manager').id)])
        ...
        elif self.env.user.has_group('aicia_portal_requests.group_director_manager') and self.status == 'approved_director':
            self.status = 'approve'
            #CREATE LA FACTURA EN BLANCO CON EL ADJUNTO Y LA DISTRIBUCION CONTABLE
            self._create_purchase_invoice()
            ...
            if self.equip_boss == self.user_id:
                user_to_notify = self.env['res.users'].search([('groups_id', 'in', self.env.ref('aicia_portal_requests.group_personnel_purchase_responsible').id)])
                self.status = 'approved_purchase_responsible'
            else:
