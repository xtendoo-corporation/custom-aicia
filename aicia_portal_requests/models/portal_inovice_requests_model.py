...
class PortalInvoiceRequest(models.Model):
    ...
    def _compute_show_solicitar_revision(self):
        is_boss = self.env.user.sudo().has_group('aicia_portal_requests.group_equip_boss') or self.env.user.sudo().has_group('aicia_portal_requests.group_intern_partner_responsible')
        for rec in self:
    ...
    def action_approve(self):
        ...
        if self.status=='approved_by_boss_group':
            ...
            user_to_send = self.env['res.users'].search([('work_group_ids', 'in', self.env.ref('aicia_portal_requests.group_intern_partner_responsible').id)])
