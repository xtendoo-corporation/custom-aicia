...
class PortalInvoiceController(Controller):
    ...
    def project_request_form(self, **kwargs):
        ...
        return request.render('aicia_portal_requests.portal_project_request_template', {
    ...
    def send_project_email(self, project):
        group = request.env.ref('aicia_portal_requests.group_director_investigation_and_development').sudo()
        admin_users = group.user_ids
        # admin_users = request.env['res.users'].search(
        #     [('groups_id', 'in', request.env.ref('aicia_portal_requests.group_director_investigation_and_development').id)])
