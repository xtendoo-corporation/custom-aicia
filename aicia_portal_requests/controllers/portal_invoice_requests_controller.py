...
class PortalInvoiceController(Controller):
    ...
    def invoice_request_form(self, **kwargs):
        ...
        if allowed_work_groups and user.sudo().has_group('aicia_portal_requests.group_equip_boss'):
        ...
        options = request.env['account.move']._fields['l10n_es_edi_facturae_reason_code']._description_selection(
            request.env)
        return request.render('aicia_portal_requests.portal_invoice_request_template', {
    ...
    def invoice_request_submit(self, **post):
        ...
        if work_group_id.equip_boss.id == invoice_request.user_id.id:
            group = request.env.ref('aicia_portal_requests.group_intern_partner_responsible').sudo()
            to_notify_users = group.user_ids
            # to_notify_users = request.env['res.users'].search(
            #     [('groups_id', 'in', request.env.ref('aicia_portal_requests.group_intern_partner_responsible').id)])
            if to_notify_users:
    ...
    def portal_my_invoice_detail(self, invoice_id, **kwargs):
        ...
        # raw_messages = ...
        # messages = ...
        return request.render('aicia_portal_requests.portal_my_invoice_detail', {
