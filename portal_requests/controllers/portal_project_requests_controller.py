...
class PortalInvoiceController(Controller):
    ...
    def send_project_email(self, project, signed_contract_data, signed_contract_filename, budget_file_data,
        ...
        admin_users = request.env['res.users'].search([('groups_id', 'in', request.env.ref('portal_requests.group_director_investigation_and_development').id)])
        print("ADMIN USER")
        print(admin_users)
